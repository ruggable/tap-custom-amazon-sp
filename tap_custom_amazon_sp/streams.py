...
class ListFinancialEventsEntity(AmazonSellerStream):
    """Define custom stream."""

    name = "list-financial-events-entity"
    primary_keys = ["AmazonOrderId"]
    replication_key = "PostedDate"
    #records_jsonpath = "$.[ShipmentEventList][*][*]" # handling path inside API call function
...
class ListFullFinancialEvents(AmazonSellerStream):
    """Define custom stream."""

    name = "list-full-financial-events"
    #primary_keys = ["AmazonOrderId"] - PK is inside entities
    #replication_key = "PostedDate" - replication key is inside entities
    #records_jsonpath = "$.FinancialEvents[*]" # - handling path inside API call function

    # Optionally, you may also use `schema_filepath` in place of `schema`:
    schema_filepath = SCHEMAS_DIR / "list-financial-events-full.json" 
